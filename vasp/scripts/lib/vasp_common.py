#!/usr/bin/env python3
"""
vasp_common.py — VASP opt 工作流公共模块（共享工具 + opt 相关）
...
功能:
  ...
  - POSCAR 生成
  - 体系文件夹建立（vasp/<A>/<B>/opt 与 scf_ELF 并列）

scf_ELF 相关功能已拆分至 scripts/lib/scf_common.py。

脚本安装目录（可整体 deploy 到任意路径）:
  vasp/scripts/lib/          — 公共库
  vasp/scripts/opt/          — opt 入口脚本
  vasp/scripts/scf_ELF/      — scf_ELF 入口脚本

计算工作目录（与脚本路径无关，由 --work-root 或 VASP_WORK_ROOT 指定）:
  <work_root>/<A>/<B>/opt/       — 结构优化
  <work_root>/<A>/<B>/scf_ELF/   — SCF + ELF
...
def system_base_dir(work_root: str | Path, material: str, pressure: str) -> Path:
    """Return <work_root>/<material>/<pressure>/ base path."""
    return Path(work_root) / material / pressure


def get_work_dirs(base_dir: str | Path) -> Tuple[Path, Path]:
    """Return (opt, scf_ELF) paths under a system base directory."""
    base = Path(base_dir)
    return base / OPT_SUBDIR, base / SCF_SUBDIR


def opt_work_dir(base_dir: str | Path) -> Path:
    """Return path to opt work directory under a system base."""
    return Path(base_dir) / OPT_SUBDIR


def iter_opt_work_dirs(work_root: str | Path) -> List[Path]:
    """List opt work dirs at work_root/<material>/<pressure>/opt/."""
    root = Path(work_root)
    return sorted(d for d in root.glob(f"*/*/{OPT_SUBDIR}") if d.is_dir())


def iter_opt_input_files(work_root: str | Path, filename: str) -> List[Path]:
    """List files at work_root/<material>/<pressure>/opt/<filename>."""
    root = Path(work_root)
    return sorted(p for p in root.glob(f"*/*/{OPT_SUBDIR}/{filename}") if p.is_file())


def write_run_script(
    target_dir: str | Path,
    incar: str,
    filename: str = OPT_PBS_SCRIPT,
) -> Path:
...
def setup_opt_scripts(base_dir: str | Path) -> Path:
    """Create opt directory and write opt (Relaxation) run script."""
    opt_dir = Path(base_dir) / OPT_SUBDIR
    write_run_script(opt_dir, OPT_INCAR, filename=OPT_PBS_SCRIPT)
    return opt_dir


def deploy_poscar_to_opt(structure: RelaxStructure, base_dir: str | Path) -> Path:
    """Write POSCAR into opt only."""
    opt_dir = Path(base_dir) / OPT_SUBDIR
...
def deploy_opt_system(
    ...
    """Full deploy for opt: run script + POSCAR + POTCAR in opt."""
...
def write_opt_pbs(
    ...
    """Write relaxation PBS script to given directory."""
    return write_run_script(target_dir, OPT_INCAR, filename=filename)
