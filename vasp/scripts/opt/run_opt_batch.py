#!/usr/bin/env python3
"""
run_opt_batch.py — opt 高通量一键批量部署

目录结构:
  <work_root>/<A>/<B>/opt/   Relaxation INCAR

基本用法:
  python run_opt_batch.py /path/to/QE_folder --work-root /path/to/calc
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Batch deploy opt calculations from QE folders."
    )
    ...
    parser.add_argument(
        "--skip-existing",
        action="store_true",
        help="Skip if POSCAR and POTCAR already exist in opt",
    )
...
def process_one(
    ...
    try:
        ...
        print(f"  {OPT_SUBDIR}/{OPT_PBS_SCRIPT} written (Relaxation INCAR)")
